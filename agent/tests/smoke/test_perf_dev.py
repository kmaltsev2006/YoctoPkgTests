...
@allure.suite('perf-dev tests')
@pytest.mark.smoke
@pytest.mark.perf_dev
class TestPerfDev:
    '''
    perf-dev smoke test class
    Tests are similar to perf tests because
    perf-dev package is RPROVIDED by perf package,
    so perf-dev is an alias to perf
    '''
    @allure.title('perf-dev: installed version test')
    @pytest.mark.minimal
    ...
    @allure.title('perf-dev: basic perf list test')
    @pytest.mark.minimal
    ...
    @allure.title('perf-dev: perf stat test')
    @pytest.mark.minimal
